...
export const wmsService = {
  ...
  async syncRequests() {
    ...
    if (!mrSpreadsheetId) throw new Error("Material requests spreadsheet ID is not configured.");

    let mrId = mrSpreadsheetId;
    let mrGid = "193399218";
    if (mrId.includes("docs.google.com/spreadsheets")) {
      const gidMatch = mrId.match(/[?&]gid=([^&#]+)/);
      if (gidMatch) mrGid = gidMatch[1];
      const match = mrId.match(/\/d\/([^/]+)/);
      if (match) mrId = match[1];
    }

    const url = `https://docs.google.com/spreadsheets/d/${mrId}/export?format=csv&gid=${mrGid}`;
    const res = await fetch(url);
    if (!res.ok) throw new Error("Failed to fetch spreadsheet from Google Sheets.");
    const text = await res.text();
    const { parseCSV } = await import("../utils/csv.js");
    const { parseSafeDate } = await import("../utils/date.js");
    const mrRows = parseCSV(text).slice(1);
    ...
    for (const row of mrRows) {
      ...
      const existingMR = await prisma.materialRequest.findFirst({
        where: {
      ...
      if (existingMR) continue; // Already imported
      ...
      if (appId) {
        ...
        if (complaint && complaint.tickets && complaint.tickets.length > 0) {
          ...
          // Create a dummy complaint and ticket
    ...
    return { newRequestsImported: newRequestsCount };
  }
};
