...
var CONFIG = {
  ...
  // Sync IDs
  SOURCE_SPREADSHEET_ID: "1Bs2jVPrm0g__cIOg8V80BbGhI6gzqCnw-bYAPissV8s",
  DESTINATION_SPREADSHEET_ID: "10S6_Nec4hsdZqOBARjAAe10P4q6F_S2NYkyVAv20pCM",

  // Optional: delete sheets in Destination that no longer exist in Source
  DELETE_ORPHAN_SHEETS: false
};
 ...
 * preserving formatting, dropdowns, filters, and frozen rows.
 * Triggers backend database reload ONLY after successful copy.
 */
function syncAllSheets() {
  Logger.log("🔄 Starting sheet synchronization from Source to Destination...");
  var startTime = new Date();

  var sourceSS = SpreadsheetApp.openById(CONFIG.SOURCE_SPREADSHEET_ID);
  var destSS = SpreadsheetApp.openById(CONFIG.DESTINATION_SPREADSHEET_ID);

  var sourceSheets = sourceSS.getSheets();
  var destSheets = destSS.getSheets();

  var sourceSheetNames = sourceSheets.map(function(s) { return s.getName(); });
  ...
  for (var i = 0; i < sourceSheets.length; i++) {
    ...
    // Skip log sheets
    if (sheetName === "Execution Logs") continue;

    Logger.log("Processing sheet: " + sheetName);
    var attempts = 3;
    var copiedSuccessfully = false;

    while (attempts > 0 && !copiedSuccessfully) {
      try {
        ...
        if (values.length && values[0].length) {
          // Clear content only (preserves conditional formatting, dropdowns, frozen rows, widths)
          var destLastRow = destSheet.getLastRow();
          var destLastCol = destSheet.getLastColumn();
          if (destLastRow > 0 && destLastCol > 0) {
            destSheet.getRange(1, 1, destLastRow, destLastCol).clearContent();
          }

          // Write new values
          destSheet.getRange(1, 1, values.length, values[0].length).setValues(values);
          totalRowsCopied += values.length;
        }

        copiedSuccessfully = true;
        successCount++;
        Logger.log("Successfully copied sheet: " + sheetName + " (" + values.length + " rows)");
      } catch (err) {
        attempts--;
        Logger.log("Failed copying " + sheetName + ". Attempt remaining: " + attempts + ". Error: " + err.toString());
        if (attempts === 0) {
          errors.push("Sheet '" + sheetName + "': " + err.toString());
        }
  ...
  // Optionally delete sheets that no longer exist in source
  if (CONFIG.DELETE_ORPHAN_SHEETS) {
    destSheets.forEach(function(s) {
      var name = s.getName();
      if (sourceSheetNames.indexOf(name) === -1 && name !== "Execution Logs") {
        try {
          destSS.deleteSheet(s);
          Logger.log("Deleted orphan sheet: " + name);
        } catch (e) {
  ...
  var duration = ((endTime.getTime() - startTime.getTime()) / 1000).toFixed(2);

  // Write execution details to Log sheet
  logExecution(destSS, startTime, endTime, totalRowsCopied, duration, errors);

  if (errors.length > 0) {
    Logger.log("⛔ Sync finished with errors: " + errors.join(", "));
    return;
  }

  Logger.log("🎉 Sync finished successfully in " + duration + "s. Triggering live backend database reload...");

  // Trigger backend reload
  triggerBackendReload();
}

/**
 * Trigger clean reload of spreadsheet data on backend Render app
 */
function triggerBackendReload() {
  var url = CONFIG.API_BASE_URL + "/sync/full";
  var options = {
    "method": "post",
    "contentType": "application/json",
...
/**
 * Log sync executions to a helper sheet named 'Execution Logs' in the destination sheet
 */
function logExecution(ss, startTime, endTime, rowsCount, duration, errors) {
  try {
    var logSheet = ss.getSheetByName("Execution Logs");
    if (!logSheet) {
      logSheet = ss.insertSheet("Execution Logs");
      logSheet.getRange("A1:F1").setValues([["Start Time", "End Time", "Duration (s)", "Rows Copied", "Status", "Errors"]]).setFontWeight("bold");
    }
    
    var status = errors.length === 0 ? "SUCCESS" : "FAILED";
    ...
    Logger.log("Could not write execution logs: " + e.toString());
  }
}
